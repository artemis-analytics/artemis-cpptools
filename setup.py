...
def main():
    ...
    make_ext(
        "artemis_cpptools.tools._filtercoltool",
        sources="./artemis_cpptools/tools/_filtercoltool.pyx",
    )
    # Any non-Cython extensions should be manually added to 'ext_modules'.

    setup(
        name="artemis_cpptools",
        version="0.1.0",
        author="Ryan White",
        ...
        install_requires=[],
        description="Arrow-based cpp tools for Artemis ecosystem",
    )
...
class build_ext_(build_ext):
    ...
    build_ext bug where if $CC is not defined as g++, -Wstrict-prototypes is passed to
    the compiler. This compiler flag is not supported in C++. The compiler will ignore
    it, but we can remove the warning by manually removing the flag.
    """
